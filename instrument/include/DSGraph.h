...
class DSScalarMap {
  ...
  ValueMapTy ValueMap;
  typedef std::unordered_set<GlobalValue*> GlobalSetTy;
...
public:
  ...
  typedef std::unordered_map<Function*, DSNodeHandle> ReturnNodesTy;
...
private:
  ...
  /// DL - This is the target data layout for an object for the machine this
  /// graph is constructed for.
  const DataLayout &DL;

  /// Context - This is the context for the module that the graph is for.
  ///
  //const LLVMContext &Context;

  /// TLI - This contains the target library functions
  ///
...
public:
  ...
  DSGraph(EquivalenceClasses<GlobalValue*> &ECs, const DataLayout &DL,
                                                 //const LLVMContext &Context,
                                                 const TargetLibraryInfo &TLI)
    : GlobalsGraph(0), PrintAuxCalls(false), ScalarMap(ECs), DL(DL),
      //Context(Context),
      TLI(TLI) {}

  // Compute the local DSGraph
  DSGraph(EquivalenceClasses<GlobalValue*> &ECs, const DataLayout &DL,
          const TargetLibraryInfo &TLI, Function &F, DSGraph *GlobalsGraph);
