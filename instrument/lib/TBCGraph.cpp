...
bool TDDataStructures::runOnModule(Module &M) {
  ...
  for(auto &F : M) {
    if (F.size() && !F.hasInternalLinkage())
...
void TDDataStructures::ComputePostOrder(Function &F,std::unordered_set<DSGraph*> &Visited,
                                        std::vector<DSGraph*> &PostOrder) {
  if(!F.size()) return;
...
void TDDataStructures::InlineCallersIntoGraph(DSGraph &DSG) {
  ...
  for (DSGraph::fc_iterator CI = DSG.fc_begin(), E = DSG.fc_end();
    ...
    if (IndCallRecI != IndCallMap.end() && IndCallRecI->first == Callees) {
      ...
      IndCallGraph = new DSGraph(DSG.getGlobalECs(), DSG.getDataLayout(), DSG.getTargetLibraryInfo());
