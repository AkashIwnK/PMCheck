...
namespace DS {
  ...
  bool isPointerType(const Type *Ty) {
    if (isa<PointerType>(Ty))
      return true;
    else if(Ty->isSized() && Ty->isIntegerTy())
      return Ty->getPrimitiveSizeInBits() >> 3 >= PointerSize;
    return false;
  }
}
...
///
  ...
  // Visitor functions, used to handle each instruction type we encounter...
    ...
    void visitMallocInst(CallInst &MI) { handleAlloc(MI, true); }
    void visitPMAllocInst(CallInst &PI) { handleAlloc(PI, false, true); }
    void visitAllocaInst(AllocaInst &AI) { handleAlloc(AI); }
    void handleAlloc(Instruction &I, bool isHeap = false,
                                     bool isPMAlloc = false);
...
// graph.
DSGraph::DSGraph(EquivalenceClasses<GlobalValue*> &ECs, const DataLayout &DL,
                 const TargetLibraryInfo &TLI, Function &F, DSGraph *GG)
  : GlobalsGraph(GG), ScalarMap(ECs), DL(DL), //Context(F.getContext()),
  TLI(TLI) {
  PrintAuxCalls = false;
  ...
  if (ScalarMap.global_begin() != ScalarMap.global_end()) {
    ...
    for (DSScalarMap::global_iterator I = ScalarMap.global_begin();
      ...
      if (GlobalVariable *GV = dyn_cast<GlobalVariable>(*I)) {
        if (!GV->isExternallyInitialized() && GV->isConstant())
...
DSNodeHandle GraphBuilder::getValueDest(Value &Val) {
  ...
  if (GlobalValue* GV = dyn_cast<GlobalValue>(V)) {
    ...
    if (ConstantExpr *CE = dyn_cast<ConstantExpr>(C)) {
      ...
      if(dyn_cast<CastInst>(CE)) {
        if (isa<PointerType>(CE->getOperand(0)->getType()))
          NH = getValueDest(*CE->getOperand(0));
        else
          NH = createNode()->setUnknownNodeMarker();
      } else if(dyn_cast<GetElementPtrInst>(CE)) {
...
///
void GraphBuilder::handleAlloc(Instruction &I, bool isHeap, bool isPMAlloc) {
  DSNode *N = createNode();
  if (isHeap)
    N->setHeapNodeMarker();
  else if(isPMAlloc)
...
void GraphBuilder::visitGetElementPtrInst(User &GEP) {
  ...
  const DataLayout &DL = Value.getNode()->getDataLayout();
  ...
  uint64_t Offset = 0;
  for(GetElementPtrInst::op_iterator I = GEP.op_begin(), E = GEP.op_end();
      I != E; ++I) {
  //for(gep_type_iterator I = gep_type_begin(GEP), E = gep_type_end(GEP);
    //   I != E; ++I) {
    if(const auto *STy = dyn_cast<StructType>((*I)->getType())) {
      uint64_t FieldNo =
           cast<ConstantInt>(*I)->getZExtValue();//I.getOperand())->getValue().getZExtValue();
      Offset += DL.getStructLayout(const_cast<StructType *>(STy))->getElementOffset(FieldNo);
    } else if(const auto *PTy = dyn_cast<PointerType>((*I)->getType())) {
      if(//!isa<Constant>(I.getOperand()) ||
         !cast<Constant>(*I)->isNullValue()) {//I.getOperand())->isNullValue()) {
    ...
    if(const SequentialType *STy = cast<SequentialType>(*I)) {
      ...
      if(ConstantSInt *CS = dyn_cast<ConstantInt>(GEP.getOperand(i))) {
        Offset += CS->getValue() * (DL.getTypeSizeInBits(CurTy) >> 3);
      } else {
        ...
        if(isa<PointerType>(STy)) {
          ...
          unsigned ElSize = DL.getTypeSizeInBits(CurTy) >> 3; // Convert to bytes
...
void GraphBuilder::visitVAArgInst(VAArgInst &I) {
  //TODO: also updates the argument
...
void GraphBuilder::visitCallSite(CallSite CS) {
        ...
        if (F->getName() == "calloc" || F->getName() == "posix_memalign" ||
          ...
          if (DSNode *Str = getValueDest(**CS.arg_begin()).getNode()) {
            ...
            if (DSNode *End = EndPtrNH.getNode()) {
              End->mergeTypeInfo(PointerType::get(Type::getInt8Ty(F->getContext()), 0),
          ...
          if (AI != E && isPointerType((*AI)->getType())) {
            ...
            if (DSNode *N = VAList.getNode()) {
              N->setReadMarker();
              N->mergeTypeInfo(PointerType::get(Type::getInt8Ty(F->getContext()), 0),
          ...
          if (DSNode *N = H.getNode()) {
            N->setModifiedMarker();
            N->mergeTypeInfo(Type::getDoubleTy(F->getContext()), H.getOffset());
          }
          return;
        } else {
          ...
          if (Warn)
            errs() << "WARNING: Call to unknown external function '"
  ...
  if (DisableDirectCallOpt || !isa<Function>(Callee)) {
    ...
    if (CalleeNode == 0) {
      errs() << "WARNING: Program is calling through a null pointer?\n"<< *I;
...
void GraphBuilder::visitFreeInst(CallInst &FI) {
  // Mark that the node is written to...
...
void GraphBuilder::MergeConstantInitIntoNode(DSNodeHandle &NH, Constant *C) {
  ...
  const DataLayout &DL = NH.getNode()->getDataLayout();
...
void GraphBuilder::mergeInGlobalInitializer(GlobalVariable *GV) {
  assert(!GV->isExternallyInitialized() && "Cannot merge in external global!");
...
bool LocalDataStructures::runOnModule(Module &M) {
  ...
  GlobalsGraph = new DSGraph(GlobalECs, DL, TLI);
  {
    GraphBuilder GGB(*GlobalsGraph);
    // Add initializers for all of the globals to the globals graph.
    for(Module::global_iterator I = M.global_begin(), E = M.global_end();
         I != E; ++I) {
      if(!(&*I)->isExternallyInitialized())
  ...
  for (auto &F : M) {
    if(F.size()) {
      auto *Graph = new DSGraph(GlobalECs, DL, TLI, F, GlobalsGraph);
      DSInfo.insert({&F, Graph});
    }
  }
  ...
  if (!ECGlobals.empty()) {
    //DEBUG(std::cerr << "Eliminating " << ECGlobals.size() << " EC Globals!\n");
    for (std::unordered_map<Function*, DSGraph*>::iterator I = DSInfo.begin(),
           E = DSInfo.end(); I != E; ++I) {
