...
DSNodeHandle &DSScalarMap::AddGlobal(GlobalValue *GV) {
  ...
  // This is either not an equivalenced global or it is the leader, it
...
DSNode::DSNode(const Type *T, DSGraph *G)
  : NumReferrers(0), Size(0), ParentGraph(G), Ty(Type::getVoidTy(T->getContext())), NodeType(0) {
...
// Get the Context object used to construct this node.
//const LLVMContext &DSNode::getContext() const {
  //return ParentGraph->getContext();
//}

void DSNode::assertOK() const {
  assert((Ty != Type::getVoidTy(Ty->getContext())//Type::getVoidTy(getContext() ||
       || Ty == Type::getVoidTy(Ty->getContext())//Type::getVoidTy(getContext()
        && (Size == 0 || (NodeType & DSNode::Array))) &&
         "Node not OK!");
...
void DSNode::forwardNode(DSNode *To, unsigned Offset) {
  ...
  NodeType = DEAD;
  Size = 0;
  Ty = Type::getVoidTy(Ty->getContext());
...
// If we determine that this node has some funny
...
// node.
  if (getSize() <= 1) {
    ...
    DestNode->Ty = Type::getVoidTy(DestNode->Ty->getContext());
    DestNode->Size = 1;
...
bool DSNode::isNodeCompletelyFolded() const {
  return getSize() == 1 && Ty == Type::getVoidTy(Ty->getContext()) && isArray();
}
...
namespace {
  ...
  public:
    TypeElementWalker(const Type *T, const DataLayout &DL) : DL(DL) {
      Stack.push_back(T);
      StepToLeaf();
    }
  ...
  private:
    /// Pop the current element off of the stack and
      ...
      while (!Stack.empty()) {
        ...
        if (const StructType *ST = dyn_cast<StructType>(SS.Ty)) {
          ...
          if (SS.Idx != ST->getNumElements()) {
            const StructLayout *SL = DL.getStructLayout(const_cast<StructType *>(ST));
            SS.Offset +=
          ...
          if (SS.Idx != AT->getNumElements()) {
            SS.Offset += (unsigned(DL.getTypeSizeInBits(AT->getElementType())) >> 3);
            return;
          }
        ...
        if (const StructType *ST = dyn_cast<StructType>(SS.Ty)) {
          ...
          if (AT->getNumElements() == 0) {
            ...
            Stack.push_back(StackState(AT->getElementType(),
                                       SS.Offset+SS.Idx*
                             unsigned(DL.getTypeSizeInBits(AT->getElementType()))));
          }
        }
      }
    }
...
static bool ElementTypesAreCompatible(const Type *T1, const Type *T2,
                                      bool AllowLargerT1, const DataLayout &DL){
  TypeElementWalker T1W(T1, DL), T2W(T2, DL);
  while (!T1W.isDone() && !T2W.isDone()) {
    ...
    if (T1 != T2 && !T1->canLosslesslyBitCastTo(const_cast<Type *>(T2)))
      return false;
...
/// This function merges the specified type into the current node
...
bool DSNode::mergeTypeInfo(const Type *NewTy, unsigned Offset,
  ...
  assert(((Size == 0 && Ty == Type::getVoidTy(Ty->getContext()) && !isArray()) ||
          (Size == 0 && !Ty->isSized() && !isArray()) ||
          (Size == 1 && Ty == Type::getVoidTy(Ty->getContext()) && isArray()) ||
          (Size == 0 && !Ty->isSized() && !isArray()) ||
          (DL.getTypeSizeInBits(const_cast<Type *>(Ty)) >> 3 == Size)) &&
         "Size member of DSNode doesn't match the type structure!");
  assert(NewTy != Type::getVoidTy(NewTy->getContext()) && "Cannot merge void type into DSNode!");
  ...
  while (const ArrayType *AT = dyn_cast<ArrayType>(NewTy)) {
    // TODO: we might want to keep small arrays, but must be careful about
  ...
  unsigned NewTySize = NewTy->isSized() ? DL.getTypeSizeInBits(const_cast<Type *>(NewTy)) >> 3: 0;
  ...
  if (Offset + NewTySize > Size) {
    ...
    if (Offset) {  // We could handle this case, but we don't for now...
      //err() << "UNIMP: Trying to merge a growth type into "
        //        << "offset != 0: Collapsing!\n";
      if (FoldIfIncompatible) foldNodeCompletely();
      return true;
    }
    ...
    // Must grow links to be the apt size.
    Links.resize(NumFields);

    // Merge in the old type now.
  ...
  while (O < Offset) {
    assert(Offset-O < DL.getTypeSizeInBits(const_cast<Type *>(SubType)) && "Offset out of range!");
    ...
    case Type::StructTyID: {
      ...
      O += (unsigned)SL.getElementOffset(i);//MemberOffsets[i];
      break;
    }
    case Type::ArrayTyID: {
      SubType = cast<ArrayType>(SubType)->getElementType();
      unsigned ElSize = (unsigned)DL.getTypeSizeInBits(const_cast<Type *>(SubType)) >> 3;
  ...
  unsigned SubTypeSize = SubType->isSized() ?
       (unsigned)DL.getTypeSizeInBits(const_cast<Type *>(SubType)) >> 3 : 0;

  // Ok, we are getting desperate now.  Check for physical subtyping, where we
  // just require each element in the node to be compatible.
  if (NewTySize <= SubTypeSize && NewTySize && NewTySize < 256 &&
      SubTypeSize && SubTypeSize < 256 &&
      ElementTypesAreCompatible(NewTy, SubType, !isArray(), DL)) {//TD))
    return false;
  }
  ...
  while (SubType != NewTy) {
    ...
    case Type::StructTyID: {
      ...
      NextSubTypeSize = (unsigned)DL.getTypeSizeInBits(const_cast<Type *>(NextSubType)) >> 3;
      break;
    }
    case Type::ArrayTyID:
  ...
  if (NewTySize == SubTypeSize) {
    ...
    if (NewTy->canLosslesslyBitCastTo(const_cast<Type *>(SubType)))
      return false;
  ...
  //DEBUG(err() << "MergeTypeInfo Folding OrigTy: ";
    //    WriteTypeSymbolic(err(), Ty, M) << "\n due to:";
      //  WriteTypeSymbolic(err(), NewTy, M) << " @ " << Offset << "!\n"
        //          << "SubType: ";
      //  WriteTypeSymbolic(err(), SubType, M) << "\n\n");
  if (FoldIfIncompatible) 
	foldNodeCompletely();
  return true;
}
...
// Efficiently merge a vector into another vector where duplicates are 
// not allowed and both are sorted.  This assumes that 'T's are
...
// Helper function for DSNode::mergeWith().
...
void DSNode::MergeNodes(DSNodeHandle& CurNodeH, DSNodeHandle& NH) {
  ...
  if (NH.getNode()->Ty != Type::getVoidTy(NH.getNode()->Ty->getContext()))
...
// Merge this node and the specified node, moving all links to and
...
void DSNode::mergeWith(const DSNodeHandle &NH, unsigned Offset) {
  ...
  if (N == this) {
    ...
    // completely.
    //DEBUG(err() << "Attempting to merge two chunks of"
...
void ReachabilityCloner::merge(const DSNodeHandle &NH,
  ...
  if (NH.getOffset() >= SrcNH.getOffset()) {
    if (!DN->isNodeCompletelyFolded()) {
      ...
      // Merge the type entries of the two nodes together...
      if (SN->getType() != Type::getVoidTy(SN->getType()->getContext())
...
DSGraph::DSGraph(const DSGraph &G, EquivalenceClasses<GlobalValue*> &ECs,
                 unsigned CloneFlags)
  : GlobalsGraph(0), ScalarMap(ECs), DL(G.DL), //Context(G.Context),
  TLI(G.TLI) {
  PrintAuxCalls = false;
...
// Allow inspection of graph in a debugger.
...
static bool isHeapAllocFunc(const FunctionType &FTy, LibFunc F,
                            const DataLayout *DL) {
  LLVMContext &Context = FTy.getContext();
  Type *PCharTy = Type::getInt8PtrTy(Context);
  Type *SizeTTy = DL ? DL->getIntPtrType(Context, /*AS=*/0) : nullptr;
...
// This function can be used to add global, stack, and heap
  ...
  if(GlobalValue *GV = dyn_cast<GlobalValue>(Ptr)) {
    ...
    if(auto *Callee = CI->getCalledFunction()) {
      ...
      if(TLI.getLibFunc(*Callee, TLIFn)) {
        if(isHeapAllocFunc(*(Callee->getFunctionType()), TLIFn, &DL)) {
          N->setHeapNodeMarker();
        }
      }
    }
...
// Clone the specified DSGraph into the current graph.  The
...
// ReturnNodes. The CloneFlags member controls various aspects of the cloning process.
...
namespace {
  // This structure is used to find nodes that have a path from the
  ...
  if (DSNode *N = Edge.getNode())  {// Is there an edge?
    if (N->getNumReferrers() == 1)  {// Does it point to a lonely node?
      ...
      if ((N->getNodeFlags() & ~DSNode::Incomplete) == 0 &&
          N->getType() == Type::getVoidTy(N->getType()->getContext())
...
static void removeIdenticalCalls(std::list<DSCallSite> &Calls) {
  ...
  //DEBUG(if (NumDeleted)
    //      err() << "Merged " << NumDeleted << " call nodes.\n";);
}
...
static bool CanReachAliveNodes(DSNode *N, std::unordered_set<const DSNode*> &Alive,
  ...
  if (IgnoreGlobals && N->isGlobalNode()) 
	return false;
...
// Use a more powerful reachability analysis to eliminate
...
void DSGraph::removeDeadNodes(unsigned Flags) {
  //DEBUG(AssertGraphOK();
  //if (GlobalsGraph) GlobalsGraph->AssertGraphOK();
...
void DSGraph::AssertCallSiteInGraph(const DSCallSite &CS) const {
  if (CS.isIndirectCall()) {
    ...
    if (CS.getNumPtrArgs() && CS.getCalleeNode() == CS.getPtrArg(0).getNode() &&
      ...
      errs() << "WARNING: WEIRD CALL SITE FOUND!\n";
#endif
  }
